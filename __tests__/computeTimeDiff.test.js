...
describe("Time diff for humans", () => {
  test("diff < 1 min", () => {
    ...
    expect(computeTimeDiff(0, "en", true).humanDiff).toEqual("< 1min");
    expect(computeTimeDiff(100).humanDiff).toEqual("less than a minute");
    expect(computeTimeDiff(1000, "en", true).humanDiff).toEqual("< 1min");
  });

  test("minutes", () => {
    expect(computeTimeDiff(10 * 60 * 1000).humanDiff).toEqual("10 minutes");
    expect(computeTimeDiff(9 * 60 * 1000, "en", true).humanDiff).toEqual("9 mins");
  });

  test("hours", () => {
    expect(computeTimeDiff(2 * 60 * 60 * 1000).humanDiff).toEqual("2 hours");
    expect(computeTimeDiff(12 * 60 * 60 * 1000, "en", true).humanDiff).toEqual("12 hrs");
  });

  test("days", () => {
    expect(computeTimeDiff(1 * 24 * 60 * 60 * 1000).humanDiff).toEqual("1 day");
    expect(computeTimeDiff(5 * 24 * 60 * 60 * 1000, "en", true).humanDiff).toEqual("5 days");
  });
...
describe("Compute nextUpdate", () => {
  test("final update", () => {
    ...
    expect(computeTimeDiff(0, "en", true).timeLeftForNextUpdate).toEqual(0);
    expect(computeTimeDiff(100).timeLeftForNextUpdate).toEqual(0);
    expect(computeTimeDiff(1000, "en", true).timeLeftForNextUpdate).toEqual(0);
  });

  test("when some time is left", () => {
    expect(computeTimeDiff(3540000, "en", true).timeLeftForNextUpdate).toEqual(3480000);
    expect(computeTimeDiff(3540061, "en", true).timeLeftForNextUpdate).toEqual(3480000);
  });
});
